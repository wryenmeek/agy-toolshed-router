...
- Antigravity (AGY) Google Gemini backend adapter and headless streaming bridge (`claude-agy-gemini-3.6-flash`, `claude-agy-gemini-3.1-pro`, `claude-agy-gpt-oss-120b`). Automatically maps Anthropic Messages API requests and tool use to Google Gemini REST streaming format using local `agy` CLI authentication or `GEMINI_API_KEY`.
- Automated onboarding wizard (`setup`) featuring pre-flight prerequisite and dependency audit checks (Node.js version >= 22.5.0, AGY / Gemini credentials, `claude-code-proxy` binary, Grok CLI, port availability on 18764/18765, and shadowed `ANTHROPIC_BASE_URL` detection), interactive TTY scope selection (`project-local` vs `user-global`), and non-interactive CLI flags (`--scope project|user`, `--interactive`, `--yes`).
- Discover AGY Gemini, GPT-OSS, and Claude quota models dynamically through `agy models`, expose AGY Claude picker rows, and route them through the AGY CLI.

### Fixes

- Resolve fixture temporary directory realpath on macOS in wiring-mode tests to prevent `/var` vs `/private/var` symlink assertion mismatches.
- Ensure clean SIGINT and readline interface teardown in setup wizard terminal prompting.
- Properly abort upstream AGY CLI child process streams when clients disconnect.
- Bound AGY CLI request and discovery execution, with SIGTERM/SIGKILL cleanup, HTTP 504 timeout errors, terminal streaming errors, zero-exit unsuccessful-result errors, canonical executable validation, and filtered child environments.
- Report AGY authentication readiness from `ensure`, `status`, and `doctor` using a bounded non-interactive `agy models` probe, and include AGY rows and provider readiness in the Sidequest catalog.
