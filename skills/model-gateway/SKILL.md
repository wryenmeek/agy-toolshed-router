...
- `ensure`, `status`, and `doctor` report AGY authentication readiness. Gemini/API catalog rows accept `GEMINI_API_KEY`; any catalog containing AGY Claude quota rows requires a successful bounded non-interactive `agy models` probe and an authenticated AGY CLI. Provider readiness is recomputed when catalogs refresh or merge a subset, including preserved AGY rows, so partial model responses cannot leave stale status.
