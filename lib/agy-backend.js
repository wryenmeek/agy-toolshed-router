...
const AGY_MODEL_LIST_TIMEOUT_MS = Number(process.env.CODEX_GATEWAY_AGY_MODELS_TIMEOUT_MS) > 0
  ? Math.min(Number(process.env.CODEX_GATEWAY_AGY_MODELS_TIMEOUT_MS), 60000)
  : 10000;
const AGY_CLI_TIMEOUT_MS = Number(process.env.CODEX_GATEWAY_AGY_TIMEOUT_MS) > 0
  ? Math.min(Number(process.env.CODEX_GATEWAY_AGY_TIMEOUT_MS), 3600000)
  : 3600000;
const AGY_DISCOVERY_OUTPUT_LIMIT = Number(process.env.CODEX_GATEWAY_AGY_DISCOVERY_OUTPUT_LIMIT) > 0
  ? Math.min(Number(process.env.CODEX_GATEWAY_AGY_DISCOVERY_OUTPUT_LIMIT), 1024 * 1024)
  : 256 * 1024;
const AGY_MODEL_ID_RE = /^(?:gemini|gpt-oss|claude)-[a-z0-9][a-z0-9.-]*$/i;

function resolveAgyCliPath(binPath = process.env.CODEX_GATEWAY_AGY_BIN || 'agy', {
  env = process.env, fsImpl = fs,
} = {}) {
  const requested = String(binPath || 'agy');
  const candidates = path.isAbsolute(requested) || requested.includes(path.sep)
    ? [requested]
    : String(env.PATH || '').split(path.delimiter).filter(Boolean).flatMap((dir) => {
      const names = process.platform === 'win32' && !path.extname(requested)
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (process.platform !== 'win32') {
        if ((stat.mode & 0o111) === 0 || (stat.mode & 0o022) !== 0) continue;
        if (typeof process.getuid === 'function' && stat.uid !== process.getuid() && stat.uid !== 0) continue;
      }
      return real;
    } catch {}
  }
...
function agyChildEnv(env = process.env) {
  const allowed = new Set([
    'PATH', 'HOME', 'USERPROFILE', 'APPDATA', 'LOCALAPPDATA', 'XDG_CONFIG_HOME',
    'GEMINI_API_KEY', 'GOOGLE_APPLICATION_CREDENTIALS', 'CODEX_GATEWAY_AGY_HOME',
    'CODEX_GATEWAY_AGY_BIN', 'AGY_HOME', 'AGY_CONFIG_DIR', 'AGY_ARGS_FILE', 'NODE_PATH',
  ]);
  return Object.fromEntries(Object.entries(env).filter(([key, value]) => allowed.has(key) && value != null));
}
...
function checkAgyCli(binPath = process.env.CODEX_GATEWAY_AGY_BIN || 'agy', { spawnSyncImpl = spawnSync, env = process.env } = {}) {
  const resolved = spawnSyncImpl === spawnSync ? resolveAgyCliPath(binPath) : binPath;
  if (!resolved) return { present: false, version: null };
  try {
    const result = spawnSyncImpl(resolved, ['--version'], {
      encoding: 'utf8', timeout: 5000, windowsHide: true,
      ...(process.platform === 'win32' && /\.(?:cmd|bat)$/i.test(resolved) ? { shell: true } : {}),
      env: agyChildEnv(env),
    });
    if (result.status === 0 && result.stdout) return { present: true, version: result.stdout.trim(), path: resolved };
  } catch {}
  return { present: false, version: null };
}

function checkAgyAuth(binPath = process.env.CODEX_GATEWAY_AGY_BIN || 'agy', {
  spawnSyncImpl = spawnSync, fsImpl = fs, env = process.env, requireCli = false,
} = {}) {
  const cliCheck = checkAgyCli(binPath, { spawnSyncImpl, env });
  if (cliCheck.present) {
    try {
      const result = spawnSyncImpl(cliCheck.path || binPath, ['models'], {
        encoding: 'utf8', timeout: AGY_MODEL_LIST_TIMEOUT_MS, windowsHide: true,
        maxBuffer: AGY_DISCOVERY_OUTPUT_LIMIT,
        ...(process.platform === 'win32' && /\.(?:cmd|bat)$/i.test(cliCheck.path || binPath) ? { shell: true } : {}),
        env: agyChildEnv(env),
      });
      if (result.status === 0) return { present: true, type: 'agy_cli', version: cliCheck.version };
    } catch {}
    if (requireCli) {
      if (env.GEMINI_API_KEY) return { present: true, type: 'api_key' };
  ...
  if (fsImpl.existsSync(adcPath)) return { present: true, type: 'adc', path: adcPath };
  return { present: false, type: null };
}

function agyModelRequiresCli(id) {
  const cleanId = String(id || '').replace(/\[1m\]$/i, '');
  return /^(?:claude-agy-claude-|claude-gemini-claude-)/i.test(cleanId);
}
...
function agyPickerId(model) {
  return String(model || '').replace(/^agy-/i, '').replace(/^gemini-/i, 'gemini-');
}

function agyPickerModelId(model) {
  const id = typeof model === 'string' ? model : model?.id;
  if (typeof id !== 'string' || !id) return '';
  return /^claude-/i.test(id) && !/^claude-agy-/i.test(id)
    ? `claude-agy-${id}`
    : id;
}
...
function agyModelFromPicker(id, models = DEFAULT_AGY_MODELS.map((m) => m.id)) {
  ...
  const suffix = id.match(/(?:\[1m\])+$/i)?.[0] || '';
  if (suffix && suffix.toLowerCase() !== '[1m]') return null;
  const cleanId = suffix ? id.slice(0, -suffix.length) : id;
  let modelId = cleanId;
  if (/^claude-agy-/i.test(cleanId)) modelId = cleanId.slice('claude-agy-'.length);
  else if (/^claude-gemini-/i.test(cleanId)) modelId = cleanId.slice('claude-gemini-'.length);
  else if (/^claude-/i.test(cleanId)) modelId = cleanId.slice('claude-'.length);
  if (!AGY_MODEL_ID_RE.test(modelId)) return null;
  const match = models.find((m) => typeof m === 'string'
    && (m === modelId || m.toLowerCase() === modelId.toLowerCase() || normalizeModelSlug(m) === modelId.toLowerCase()));
  ...
  return {
    event(ev) {
      ...
      if (ev.event === 'step_update') {
        ...
        const res = ev.result;
        if (res?.status === 'SUCCESS') stop(res?.usage, 'end_turn');
        else emitError(new Error('AGY CLI returned an unsuccessful result'));
      }
    },
    end(usage) {
      if (!stopped) stop(usage, 'end_turn');
...
function discoverAgyModels({
  ...
  outputLimit = AGY_DISCOVERY_OUTPUT_LIMIT,
} = {}) {
  const safeFallback = Array.isArray(fallback) && fallback.length ? fallback : DEFAULT_AGY_MODELS;
  const resolvedBin = spawnImpl === spawn ? resolveAgyCliPath(binPath) : binPath;
  if (!resolvedBin) return Promise.resolve(safeFallback);
  return new Promise((resolve) => {
    let child;
    try {
      child = spawnImpl(resolvedBin, ['models'], {
        stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true,
        ...(process.platform === 'win32' && /\.(?:cmd|bat)$/i.test(resolvedBin) ? { shell: true } : {}),
        ...(spawnImpl === spawn ? { env: agyChildEnv() } : {}),
      });
    } catch { resolve(safeFallback); return; }
    if (!child || typeof child.on !== 'function') { resolve(safeFallback); return; }
    let stdout = '', stderr = '', settled = false, childExited = false, killTimer;
    const cap = Math.max(1024, Number(outputLimit) || AGY_DISCOVERY_OUTPUT_LIMIT);
    const terminate = () => {
      try { child.kill?.('SIGTERM'); } catch {}
      clearTimeout(killTimer);
      killTimer = setTimeout(() => { if (!childExited) try { child.kill?.('SIGKILL'); } catch {} }, 250);
    };
    const finish = (models) => {
      if (settled) return;
      settled = true;
    ...
    const onData = (target, chunk) => {
      ...
      if (target.length + text.length > cap) { terminate(); finish([]); return ''; }
      return target + text;
    };
    const timer = setTimeout(() => { terminate(); finish([]); }, Math.max(1, Number(timeout) || AGY_MODEL_LIST_TIMEOUT_MS));
    child.stdout?.on('data', (chunk) => { stdout = onData(stdout, chunk); });
    child.stderr?.on('data', (chunk) => { stderr = onData(stderr, chunk); });
    child.stdout?.on('error', () => { terminate(); finish([]); });
    child.stderr?.on('error', () => { terminate(); finish([]); });
    child.on('error', () => { clearTimeout(killTimer); terminate(); finish([]); });
    child.on('close', (code) => {
      childExited = true;
      clearTimeout(killTimer);
