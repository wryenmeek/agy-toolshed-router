...
function agyPickerModelId(model) {
  ...
  return id.startsWith('claude-') && !id.startsWith('claude-agy-')
    ? `claude-agy-${id}`
    : id;
}

function agyModelFromPicker(id, models = DEFAULT_AGY_MODELS.map((m) => m.id)) {
  const cleanId = String(id || '').replace(/\[1m\]$/, '');
  const modelId = cleanId.startsWith('claude-agy-')
    ? cleanId.slice('claude-agy-'.length)
    : cleanId.replace(/^claude-/, '');
  const match = models.find((m) => m === modelId || normalizeModelSlug(m) === modelId);
