...
function agyModelFromPicker(id, models = DEFAULT_AGY_MODELS.map((m) => m.id)) {
  ...
  const suffixes = id.match(/(?:\[1m\])+$/i)?.[0] || '';
  if (suffixes && suffixes.toLowerCase() !== '[1m]' || /\[1m\]/i.test(id.slice(0, -suffixes.length))) return null;
  const cleanId = suffixes ? id.slice(0, -suffixes.length) : id;
  let modelId = cleanId;
