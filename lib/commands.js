...
function agyReadinessReport({ requireCli = false } = {}) {
  try {
    const auth = agyBackend.checkAgyAuth(undefined, { requireCli });
    if (requireCli && auth.type !== 'agy_cli') {
      return { ready: false, state: 'auth-missing', message: 'AGY Claude quota models require an authenticated `agy` CLI.' };
    }
    return auth.present
...
async function statusReport({ readiness = null, includeAgyReadiness = true } = {}) {
  ...
  if (includeAgyReadiness) {
    agyReadiness = agyReadinessReport();
...
async function doctor({ readiness: suppliedReadiness = null } = {}) {
  ...
  const agyReadiness = agyReadinessReport();
  log(`AGY readiness: ${agyReadiness.state}`);
  if (!agyReadiness.ready) log(agyReadiness.message);
...
function providerCatalogReadiness(provider, readiness, models = []) {
  ...
  if (provider === 'agy') {
    ...
    return agyReadinessReport({ requireCli: requiresCli });
  }
...
function mergeSubsetCatalog(existing, catalog) {
  ...
  if (models.some((model) => model.provider === 'agy')) {
    const requiresCli = models.some((model) => model.provider === 'agy' && agyBackend.agyModelRequiresCli(model.id));
    providers.agy = agyReadinessReport({ requireCli: requiresCli });
  }
...
const commands = {
  ...
  stop: async () => {
    ...
    const agyReadiness = quiet ? null : agyReadinessReport();
    const reportAgyReadiness = () => {
      if (!quiet) {
        log(`AGY readiness: ${agyReadiness.state}`);
        if (!agyReadiness.ready) log(agyReadiness.message);
      }
    ...
    if (!quiet) {
      reportAgyReadiness();
      await statusReport({ readiness, includeAgyReadiness: false });
    }
    flushHookOutput();
  },
