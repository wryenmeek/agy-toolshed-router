...
async function promptScopeSelection({ defaultScope = 'project', interactive = false } = {}) {
  // Check CLI flag overrides first: e.g. --scope user or --scope project
  const scopeArgIndex = args.indexOf('--scope');
  if (scopeArgIndex !== -1 && args[scopeArgIndex + 1]) {
    const explicitScope = args[scopeArgIndex + 1].toLowerCase();
    if (explicitScope === 'user' || explicitScope === 'project') return explicitScope;
  }
  if (flag('--write-user')) return 'user';
  if (flag('--write-project')) return 'project';
  ...
  if ((!interactive && !flag('--interactive')) || !isTTY || flag('--yes')) {
    return defaultScope;
  }
...
async function doctor({ readiness: suppliedReadiness = null } = {}) {
  ...
  try {
    const agyAuth = agyBackend.readAgyAuth();
    const agyCli = agyBackend.checkAgyCli();
    log(`agy auth: present (${agyAuth.type}${agyCli.version ? `, cli ${agyCli.version}` : ''})`);
  } catch (error) {
    log(`agy auth: ${error.message}`);
  }
