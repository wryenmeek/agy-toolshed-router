...
function gatewayBackendModelId(id) {
  ...
  const clean = id.replace(/\[1m\]$/i, '');
  if (clean.toLowerCase().startsWith(AGY_PREFIX)) return clean.slice(AGY_PREFIX.length);
  return clean.replace(/^claude-/i, '');
}
...
function resolveGatewayModelPolicy(id) {
  ...
  const lowerBackendId = backendId.toLowerCase();
  const isAgy = lowerId.startsWith(AGY_PREFIX)
    || lowerId.startsWith('claude-gemini-')
    || lowerBackendId.startsWith('gemini-')
    || lowerBackendId.startsWith('gpt-oss-')
    || lowerBackendId.startsWith('claude-');
  const policyId = lowerBackendId.startsWith('gpt-') && !isAgy ? backendId.replace(/-fast$/, '') : backendId;
  const policy = MODEL_WINDOW_POLICY[policyId]
    || (lowerBackendId.startsWith('gpt-') && !isAgy ? MODEL_WINDOW_POLICY.default : null)
    || (isAgy ? MODEL_WINDOW_POLICY['agy-default'] : null)
    || (typeof id === 'string' && lowerId.startsWith(PREFIX) && !lowerId.startsWith(LEGACY_CODEX_PREFIX)
...
function gatewayDiscoveryModels(models) {
  ...
  return models.flatMap((model) => {
    if (!model || typeof model.id !== 'string' || !(/^(?:claude|anthropic|gemini|gpt-oss)-/i.test(model.id))) return [];
