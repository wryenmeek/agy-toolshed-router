...
function providerCatalogReadiness(provider, readiness, models = []) {
  ...
  if (provider === 'agy') {
    const requiresCli = models.some((model) => model.provider === 'agy' && /^claude-agy-claude-/i.test(model.id));
...
function mergeSubsetCatalog(existing, catalog) {
  ...
  if (models.some((model) => model.provider === 'agy' && /^claude-agy-claude-/i.test(model.id))) {
    providers.agy = getAgyReadiness({ requireCli: true });
  }
