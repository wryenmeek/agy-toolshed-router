...
function gatewayModel(id, backend = 'codex', details = null) {
  const policy = id === 'auto' ? null : resolveGatewayModelPolicy(id);
  if (id !== 'auto' && policy?.backend !== backend) return null;
  const providedDisplayName = details?.displayName || details?.display_name;
  return {
    id: id === 'auto' ? DISPATCH_MODEL_ID : gatewayClientModelId(id),
    display_name: id === 'auto' ? 'Sidequest Dispatch (Codex)' : providedDisplayName || displayName(id, backend),
    type: 'model',
...
function getAgyReadiness({ readAuth = agyBackend.checkAgyAuth, requireCli = false } = {}) {
  try {
    const auth = readAuth(undefined, { requireCli });
    if (requireCli && auth?.type !== 'agy_cli') {
      throw new Error('AGY Claude quota models require an authenticated `agy` CLI.');
    }
    if (auth?.present) {
      return {
        ready: true,
    ...
    throw new Error('Antigravity authentication is unavailable. Run `agy` and log in, or export GEMINI_API_KEY.');
  } catch (error) {
...
function providerCatalogReadiness(provider, readiness, models = []) {
  ...
  if (provider === 'agy') {
    const requiresCli = models.some((model) => model.provider === 'agy' && agyBackend.agyModelRequiresCli(model.id));
    return getAgyReadiness({ requireCli: requiresCli });
  }
...
function buildCatalog(ids, readiness = null) {
  ...
  const providers = Object.fromEntries(
    [...new Set(models.map((model) => model.provider))].map((provider) => [provider, providerCatalogReadiness(provider, readiness, models)]),
  );
  return {
...
function mergeSubsetCatalog(existing, catalog) {
  ...
  if (models.some((model) => model.provider === 'agy')) {
    const requiresCli = models.some((model) => model.provider === 'agy' && agyBackend.agyModelRequiresCli(model.id));
    providers.agy = getAgyReadiness({ requireCli: requiresCli });
  }
...
function runWorker() {
  ...
  async function refreshModels({ logSentryPolicies = false } = {}) {
    ...
    const agyModels = await agyBackend.discoverAgyModels();
    ...
    modelCache = {
      ...
      data: [
        ...
        ...advertisedAgyModels
          .map((model) => ({ model, pickerId: agyBackend.agyPickerModelId(model) }))
          .filter(({ pickerId }) => resolveGatewayModelPolicy(pickerId)?.backend === 'agy')
          .map(({ model, pickerId }) => gatewayModel(pickerId, 'agy', model)),
      ],
    };
  ...
  async function forwardAgy(clientReq, clientRes, payload, model, advertisedModel, routeTelemetry, usageCapture) {
    ...
    // AGY Claude quota models are CLI-backed; a Gemini API key cannot route them.
    const cliOnlyModel = model.startsWith('claude-');
    if (cliOnlyModel && auth.type === 'api_key') {
      const cliCheck = agyBackend.checkAgyCli();
      if (!cliCheck.present) {
        const body = JSON.stringify({ type: 'error', error: { type: 'authentication_error', message: 'model-gateway: AGY Claude models require the installed `agy` CLI; GEMINI_API_KEY only supports Gemini API models.' } });
        routeTelemetry?.finish(401, 'authentication_error');
        clientRes.writeHead(401, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) });
        clientRes.end(body);
        return;
    ...
    // Direct HTTP path if GEMINI_API_KEY is available for Gemini models.
    if (auth.type === 'api_key' && !cliOnlyModel) {
    ...
    const cliBin = agyBackend.resolveAgyCliPath();
    if (!cliBin) {
      const body = JSON.stringify({ type: 'error', error: { type: 'api_error', message: 'model-gateway: AGY CLI executable is missing or unsafe' } });
      routeTelemetry?.finish(502, 'upstream_error');
      clientRes.writeHead(502, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) });
      clientRes.end(body);
      return;
    }
    const cliArgs = [
    ...
    let child;
    try {
      child = spawn(cliBin, cliArgs, {
        cwd: '/tmp',
        env: agyBackend.agyChildEnv(),
        windowsHide: true,
        ...(process.platform === 'win32' && /\.(?:cmd|bat)$/i.test(cliBin) ? { shell: true } : {}),
        stdio: ['pipe', 'pipe', 'pipe'],
      });
    } catch (error) {
      routeTelemetry?.finish(502, 'upstream_error');
      const body = JSON.stringify({ type: 'error', error: { type: 'api_error', message: `model-gateway: AGY CLI spawn failed: ${error.message}` } });
      clientRes.writeHead(502, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) });
      clientRes.end(body);
      return;
    }
    ...
    const cleanup = () => {
      if (cliSettled) return;
      cliSettled = true;
      clearTimeout(timeoutTimer); clearTimeout(killTimer);
      terminate();
    };
    ...
    child.stdout.on('data', (chunk) => {
      ...
      for (;;) {
        ...
        try {
          ...
          if (ev.event === 'result') {
            resultStatus = ev.result?.status || null;
    ...
    child.stdout.on('error', (error) => child.emit('error', error));
    child.stderr.on('error', (error) => child.emit('error', error));
    ...
    child.on('error', (error) => {
      ...
      terminate();
      clientRes.removeListener('close', cleanup);
      clientReq.removeListener('aborted', cleanup);
      routeTelemetry?.finish(502, 'upstream_error');
    ...
    child.on('close', (code) => {
      ...
      clientReq.removeListener('aborted', cleanup);
      if (cliTimedOut) {
        routeTelemetry?.finish(504, 'upstream_timeout');
        const error = new Error('AGY CLI upstream timeout');
        if (streamMode) { transformer?.error(error); return clientRes.end(); }
        const errorBody = JSON.stringify({ type: 'error', error: { type: 'api_error', message: `model-gateway: ${error.message}` } });
        clientRes.writeHead(504, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(errorBody) });
        return clientRes.end(errorBody);
      }
      if (code !== 0) {
        routeTelemetry?.finish(502, 'upstream_error');
        const detail = stderr.trim() ? `: ${stderr.trim()}` : '';
        const error = new Error(`AGY CLI exited with code ${code === null ? 'signal termination' : code}${detail}`);
        if (streamMode) {
          transformer?.error(error);
          return clientRes.end();
        }
      ...
      if (resultStatus !== 'SUCCESS') {
        routeTelemetry?.finish(502, 'upstream_error');
        const error = new Error(resultStatus
          ? 'AGY CLI returned an unsuccessful result'
          : 'AGY CLI exited without a successful result');
        if (streamMode) { transformer?.error(error); return clientRes.end(); }
        const errorBody = JSON.stringify({ type: 'error', error: { type: 'api_error', message: `model-gateway: ${error.message}` } });
        clientRes.writeHead(502, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(errorBody) });
        return clientRes.end(errorBody);
    ...
    req.on('end', () => {
      ...
      if (raw && pathOnly.startsWith('/v1/messages')) {
        try {
          ...
          if (typeof parsed.model === 'string' && (/^claude-agy-/i.test(parsed.model) || /^claude-gemini-/i.test(parsed.model))) {
            const advertisedModel = parsed.model;
            ...
            if (!model) {
              const body = JSON.stringify({ type: 'error', error: { type: 'invalid_request_error', message: 'model-gateway: invalid AGY model id' } });
              routeTelemetry.finish(400, 'invalid_model');
              res.writeHead(400, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(body) });
              return res.end(body);
            }
            const effort = typeof parsed.output_config?.effort === 'string' ? parsed.output_config.effort : null;
